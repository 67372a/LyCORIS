...
VALID_PRESET_KEYS: tuple[str, ...] = (
    ...
    "exclude_name",
    # Fork extensions: per-layer regularised dims/lrs and pattern filtering.
    "network_reg_dims",
    "network_reg_lrs",
    "include_patterns",
    "exclude_patterns",
...
ALGO_REGISTRY: Dict[str, AlgoSpec] = {
    ...
    "abba": AlgoSpec(
        name="abba",
        description="SVD-free low-rank adapter (Abba).",
        supported_args=(
            "dim",
            "alpha",
            "dropout",
    ...
    "ortholora": AlgoSpec(
        name="ortholora",
        description="Orthogonal LoRA with runtime orthogonalisation.",
        supported_args=(
            "dim",
            "alpha",
            "dropout",
    ...
    "gora": AlgoSpec(
        name="gora",
        description="Gradient-driven adaptive low rank adaptation.",
        supported_args=(
            "dim",
            "alpha",
            "dropout",
        ...
        notes="Requires a precomputation phase via LycorisNetwork.prepare_gora().",
    ),
    "ralora": AlgoSpec(
        name="ralora",
        description="Rank-aligned LoRA (RaLoRA / RaLoRA-Pro).",
        supported_args=(
            "dim",
            "alpha",
            "dropout",
    ...
    "tsm": AlgoSpec(
        name="tsm",
        description="Timestep-shared mask adapter (TSM).",
        supported_args=(
            "dim",
            "alpha",
            "dropout",
        ...
        notes="Training frameworks should call set_tsm_timestep() before each forward pass.",
    ),
    "lora2": AlgoSpec(
        name="lora2",
        description="Two-stage LoRA (LoRA²) adapter.",
        supported_args=(
            "dim",
            "alpha",
            "dropout",
    ...
    def from_dict(
        ...
        for key in VALID_PRESET_KEYS:
            ...
            if key in ("module_algo_map", "name_algo_map"):
                ...
                # Fork extension keys (network_reg_dims/lrs, include/exclude
                # patterns) are preserved in `extra` and round-trip through
                # to_dict().
                extra[key] = _copy_value(data.get(key))
        preset = cls(**kwargs)
        preset.extra = extra
        return preset
...
_PRESET_FIELDS = frozenset(f.name for f in fields(PresetConfig)) - {"extra"}
