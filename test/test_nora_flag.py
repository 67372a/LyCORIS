"""CUDA regression tests for LoCon NoRA initialization and runtime behavior."""
...
def test_full_nora_normalizes_dynamically_without_mutating_parameters_and_keeps_gradients():
...
def test_nora_rebuild_bypass_and_merge_parity(kind):
    if kind == "linear":
...
def test_nora_configuration_round_trips_with_network_weights(tmp_path, suffix):
    base = nn.Linear(12, 8, bias=False).to(DEVICE)
    source = LycorisNetwork(
        nn.Identity().to(DEVICE), init_only=True, nora=True
    )
    source.add_module("adapter", _make_locon(base, nora=True))
